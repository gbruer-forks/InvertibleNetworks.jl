...
function noninvertible_layer_test_gradient(L, P, dP, X, dX; name, do_flux=nothing, tol=1e-10)
    ...
    if do_flux
        ...
        Y, back_Y = Flux.pullback(flux_forward, deepcopy(P))
        f, back_f = Flux.pullback(log_likelihood, Y)
