...
function conditional_layer_test_inverse(L, X, Cond, dX; returns_CondY=false)
    if returns_CondY
        forward_wrap = forward
    else
        forward_wrap = function (X, Cond, L)
            Y, logdet = forward(X, Cond, L)
            return Y, Cond, logdet
        end
    end
    Y, YCond, logdet = forward_wrap(X, Cond, L)
    X_ = inverse(forward_wrap(X, Cond, L)[1], YCond, L)
    @test isapprox(norm(X - X_)/norm(X), 0f0; atol=1e-5)

    X_ = forward_wrap(inverse(X, YCond, L), Cond, L)[1]
    @test isapprox(norm(X - X_)/norm(X), 0f0; atol=1e-5)
end

function conditional_layer_test_gradient(L, P, dP, X, Cond, dX; returns_CondY=false, name, do_flux=nothing, tol=1e-10)
    if returns_CondY
        forward_wrap = forward
    else
        forward_wrap = function (X, Cond, L)
    ...
    if do_flux
        ...
        flux_forward = function (P)
            set_params!(L, P)
            forward_wrap(X, Cond, L)
        end
        (Y, YCond, logdet), back_Y_YCond_logdet = Flux.pullback(flux_forward, deepcopy(P))
