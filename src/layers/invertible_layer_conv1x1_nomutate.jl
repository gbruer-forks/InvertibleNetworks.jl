...
export Conv1x1NoMutate

"""
    C = Conv1x1NoMutate(k; logdet=false)

 or

    C = Conv1x1NoMutate(v1, v2, v3; logdet=false)
...
"""
struct Conv1x1NoMutate <: NeuralNetLayer
    k::Integer
    v1::Parameter
    v2::Parameter
    v3::Parameter
...
@Flux.functor Conv1x1NoMutate

# Constructor with random initializations
function Conv1x1NoMutate(k;freeze=false, logdet=false)
    v1 = Parameter(glorot_uniform(k))
    v2 = Parameter(glorot_uniform(k))
    v3 = Parameter(glorot_uniform(k))
    return Conv1x1NoMutate(k, v1, v2, v3, logdet, freeze)
end

function Conv1x1NoMutate(v1, v2, v3;freeze=false, logdet=false)
    k = length(v1)
    v1 = Parameter(v1)
    v2 = Parameter(v2)
    v3 = Parameter(v3)
...
function conv1x1_grad_v(X::AbstractArray{T, N}, ΔY::AbstractArray{T, N},
                        C::Conv1x1NoMutate; adjoint=false) where {T, N}
...
function chain_lr_nomutate(x::AbstractMatrix{T}, vi::Vararg{AbstractVector{T}, N}) where {T, N}
    out = T(1) .* x
    for v=vi
        n = -2/dot(v, v)
        out = out + n .* out * v * v'
...
# Forward pass
function forward(X::AbstractArray{T, N}, C::Conv1x1NoMutate; logdet=nothing) where {T, N}
    ...
    Y_list = [
        (
            ...
            Yi = chain_lr_nomutate(Xi, v1, v2, v3);
            reshape(Yi, size(selectdim(X, N, i))...)
        )
        for i=1:size(X, N)
    ]
    Y = cat(Y_list...; dims=N)
...
function forward(X_tuple::Tuple, C::Conv1x1NoMutate; set_grad::Bool=true)
    ΔX = X_tuple[1]
    X = X_tuple[2]
...
# Inverse pass
function inverse(Y::AbstractArray{T, N}, C::Conv1x1NoMutate; logdet=nothing) where {T, N}
    isnothing(logdet) ? logdet = C.logdet : logdet = logdet
    X = cuzeros(Y, size(Y)...)
    n_in = size(X, N-1)
...
function inverse(Y_tuple::Tuple, C::Conv1x1NoMutate; set_grad::Bool=true)
    ΔY = Y_tuple[1]
    Y = Y_tuple[2]
